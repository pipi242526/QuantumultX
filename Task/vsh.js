...
if(!$.isNode()&&vshhd.indexOf("\n") ==-1){
    vshurlArr.push($.getdata('vshurl'))
    ...
    if($.isNode()){
    if (process.env.VSH_HD && process.env.VSH_HD.indexOf('\n') > -1) {
        vshhd = process.env.VSH_HD.split('\n');
    } else {
        vshhd = [process.env.VSH_HD]
    };
    if (process.env.VSH_URL && process.env.VSH_URL.indexOf('\n') > -1) {
        vshurl = process.env.VSH_URL.split('\n');
    } else {
        vshurl = [process.env.VSH_URL]
    };
    if (process.env.VSHDSP_BODY && process.env.VSHDSP_BODY.indexOf('\n') > -1) {
        vshdspbody = process.env.VSHDSP_BODY.split('\n');
    } else {
        vshdspbody = [process.env.VSHDSP_BODY]
    };
    if (process.env.VSHSP_BODY && process.env.VSHSP_BODY.indexOf('\n') > -1) {
        vshspbody = process.env.VSHSP_BODY.split('\n');
    } else {
        vshspbody = [process.env.VSHSP_BODY]
    };
    if (process.env.VSH1_BODY && process.env.VSH1_BODY.indexOf('\n') > -1) {
        vsh1body = process.env.VSH1_BODY.split('\n');
    } else {
        vsh1body = [process.env.VSH1_BODY]
    };
    if (process.env.VSH2_BODY && process.env.VSH2_BODY.indexOf('\n') > -1) {
        vsh2body = process.env.VSH2_BODY.split('\n');
    } else {
        vsh2body = [process.env.VSH2_BODY]
    };
    if (process.env.VSH_BODY && process.env.VSH_BODY.indexOf('\n') > -1) {
        vshbody = process.env.VSH_BODY.split('\n');
    } else {
        vshbody = [process.env.VSH_BODY]
    };
     if (process.env.VSHQD_BODY && process.env.VSHQD_BODY.indexOf('\n') > -1) {
        vshqdbody = process.env.VSHQD_BODY.split('\n');
    } else {
   ...
   vshspbody = vshspbody.split("\n")
   vsh1body = vsh1body.split("\n")
   vsh2body = vsh2body.split("\n")
   vshbody = vshbody.split("\n")
   vshqdbody = vshqdbody.split("\n")
};
    Object.keys(vshurl).forEach((item) =>{
        if (vshurl[item]) {
        vshurlArr.push(vshurl[item])
        }
    });
    Object.keys(vshhd).forEach((item) =>{
        if (vshhd[item]) {
            vshhdArr.push(vshhd[item])
        }
    });		
    Object.keys(vshdspbody).forEach((item) =>{
        if (vshdspbody[item]) {
            vshdspbodyArr.push(vshdspbody[item])
        }
    });
    Object.keys(vshspbody).forEach((item) =>{
        if (vshspbody[item]) {
        vshspbodyArr.push(vshspbody[item])
        }
    });
    Object.keys(vsh1body).forEach((item) =>{
        if (vsh1body[item]) {
        vsh1bodyArr.push(vsh1body[item])
        }
    });
    Object.keys(vsh2body).forEach((item) =>{
        if (vsh2body[item]) {
        vsh2bodyArr.push(vsh2body[item])
        }
    });
    Object.keys(vshbody).forEach((item) =>{
        if (vshbody[item]) {
        vshbodyArr.push(vshbody[item])
        }
    });
    Object.keys(vshqdbody).forEach((item) =>{
        if (vshqdbody[item]) {
        vshqdbodyArr.push(vshqdbody[item])
        }
    });
}

!(async () => {
  if (isvshck = typeof $request !== "undefined") {
    await vshck()
